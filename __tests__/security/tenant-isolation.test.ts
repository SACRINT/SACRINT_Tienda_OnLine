...
import { getTenantById, getTenantBySlug, createTenant } from "@/lib/db/tenant";
...
describe("Tenant Isolation - User Functions", () => {
  ...
  it("updateUser: should block cross-tenant updates", async () => {
    await expect(
      updateUser(testData.tenantA.id, testData.tenantB.user.id, {
        name: "Hacked Name",
      }),
  ...
  it("deleteUser: should block cross-tenant deletion", async () => {
    await expect(
      deleteUser(testData.tenantA.id, testData.tenantB.user.id),
    ).rejects.toThrow("does not belong to tenant");
  });
  ...
  it("getUsersByTenant: should only return users from specified tenant", async () => {
    ...
    expect(
      result.every((user: any) => user.id === testData.tenantA.user.id),
    ).toBe(true);
  });
  ...
  it("getProductReviews: should only return reviews from specified tenant", async () => {
    ...
    expect(
      result.reviews.every(
        (r: any) => r.productId === testData.tenantA.product.id,
      ),
    ).toBe(true);
  });
...
describe("Tenant Isolation - Tenant Functions", () => {
  it("getTenantById: should block cross-tenant access", async () => {
    await expect(getTenantById(testData.tenantB.id)).rejects.toThrow(
      "Forbidden",
    );
  });
  ...
  it("getTenantBySlug: should validate access when validateAccess=true", async () => {
    await expect(getTenantBySlug(testData.tenantB.slug, true)).rejects.toThrow(
      "Forbidden",
    );
  });
});
...
describe("Tenant Isolation - Summary", () => {
  it("should have refactored all 35 critical DAL functions", () => {
    ...
    const totalSecured = Object.values(securedFunctions).reduce(
      (sum, count) => sum + count,
      0,
    );
