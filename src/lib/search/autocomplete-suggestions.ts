...
export class AutocompleteManager {
  ...
  getAutocomplete(request: AutocompleteRequest): AutocompleteResponse {
    ...
    try {
      ...
      logger.error(
        { type: "autocomplete_error", query: request.query, error: String(error) },
  ...
  addSuggestion(suggestion: AutocompleteSuggestion): void {
    ...
    logger.debug(
      { eventType: "suggestion_added", text: suggestion.text, type: suggestion.type },
  ...
  correctTypo(misspelled: string, correct: string, confidence: number): void {
    ...
    logger.debug(
      { type: "typo_correction_added", misspelled, correct },
      `Corrección de typo: ${misspelled} → ${correct}`,
    );
  }
    ...
    return {
      ...
      averagePopularity:
        suggestions.length > 0
          ? suggestions.reduce((sum, s) => sum + s.popularity, 0) / suggestions.length
          : 0,
