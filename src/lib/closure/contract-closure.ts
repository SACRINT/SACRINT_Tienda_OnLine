...
export class ContractClosureManager {
  ...
  registerContract(
    ...
    logger.info(
      { type: "contract_registered", contractId: id },
      `Contrato registrado: ${vendor}`
    )
    return contract
  }
  ...
  finalizeContract(contractId: string): Contract | null {
    ...
    if (allCompleted) {
      ...
      logger.warn(
        { type: "contract_not_ready", contractId },
        "Contrato no listo para finalizar"
      )
    }
  ...
  assessVendor(
    ...
    const overallRating =
      (performanceRating + qualityRating + deliveryRating + communicationRating) /
      4
  ...
  getVendorAssessments(vendor: string): VendorAssessment[] {
    return Array.from(this.vendorAssessments.values()).filter(
      (a) => a.vendor === vendor
    )
  }
    ...
    return {
      ...
      averageVendorRating:
        assessments.length > 0
          ? assessments.reduce((sum, a) => sum + a.overallRating, 0) /
            assessments.length
          : 0,
    }
  }
