...
export class PostImplementationSupportManager {
  ...
  constructor() {
    logger.debug(
      { type: "post_implementation_support_init" },
      ...
      averageResolutionTimeHours:
        tickets.filter((t) => t.resolvedDate).length > 0
          ? tickets.reduce((sum, t) => {
              ...
              return (
                sum +
                (t.resolvedDate.getTime() - t.createdDate.getTime()) /
                  (1000 * 60 * 60)
              )
...
let globalPostImplementationSupportManager: PostImplementationSupportManager | null =
  null
