...
export class SuccessMetricsManager {
  ...
  recordActualValue(metricId: string, actual: number): SuccessMetric | null {
    ...
    logger.info({ type: "metric_actual_recorded", metricId }, `Valor actual registrado: ${actual}`);
    return metric;
  }
  ...
  createProjectScorecard(
    ...
    const weightedScore = metrics.reduce((sum, m) => {
      const statusValue =
        m.status === "achieved" ? 100 : m.status === "partially-achieved" ? 70 : 0;
  ...
  recordRealizedBenefit(benefitId: string, realizedValue: number): BenefitsRealization | null {
    const benefit = this.benefitsRealization.get(benefitId);
    if (!benefit) return null;

    benefit.realizedValue = realizedValue;
    benefit.realizationPercentage = (realizedValue / benefit.expectedValue) * 100 || 0;
  ...
  getStatistics(): Record<string, any> {
    ...
    return {
      ...
      metricsByStatus: {
        ...
        partiallyAchieved: metrics.filter((m) => m.status === "partially-achieved").length,
        notAchieved: metrics.filter((m) => m.status === "not-achieved").length,
      },
      totalScorecards: scorecards.length,
      scorecardsByRating: {
        highlySuccessful: scorecards.filter((s) => s.successRating === "highly-successful").length,
        successful: scorecards.filter((s) => s.successRating === "successful").length,
        partiallySuccessful: scorecards.filter((s) => s.successRating === "partially-successful")
          .length,
        unsuccessful: scorecards.filter((s) => s.successRating === "unsuccessful").length,
      },
      ...
      averageBenefitsRealization:
        benefits.length > 0
          ? benefits.reduce((sum, b) => sum + b.realizationPercentage, 0) / benefits.length
          : 0,
    };
  }
