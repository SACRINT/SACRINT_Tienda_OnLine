...
export class DataBackupVerificationManager {
    ...
    const report: VerificationReport = {
      ...
      averageIntegrity:
        verifications.length > 0
          ? verifications.reduce((sum, v) => sum + v.integrity, 0) / verifications.length
          : 0,
    };
  ...
  getStatistics() {
    return {
      ...
      passedVerifications: Array.from(this.verifications.values()).filter(
        (v) => v.status === "passed",
      ).length,
      failedVerifications: Array.from(this.verifications.values()).filter(
        (v) => v.status === "failed",
      ).length,
    };
  }
