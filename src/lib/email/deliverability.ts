...
export class DeliverabilityManager {
  ...
  setupDomain(domain: string, data: Omit<DeliverabilityConfig, 'createdAt' | 'updatedAt'>): DeliverabilityConfig {
  ...
  generateDMARCRecord(domain: string, policy: DeliverabilityConfig['dmarc']['policy']): string {
