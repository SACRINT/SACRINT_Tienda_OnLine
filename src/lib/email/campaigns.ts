...
export class EmailCampaignManager {
  ...
  private campaignSchedules: Map<string, NodeJS.Timeout> = new Map();
   ...
   * Actualizar campaña
   */
  updateCampaign(campaignId: string, updates: Partial<EmailCampaign>): EmailCampaign {
    ...
    logger.debug({ type: "campaign_updated", campaignId }, `Campaña actualizada: ${campaign.name}`);
  ...
  async sendCampaign(campaignId: string): Promise<EmailCampaign> {
    ...
    try {
      ...
      logger.error({ type: "campaign_send_failed", campaignId, error }, "Error enviando campaña");
      throw error;
    }
  }
  ...
  cancelCampaign(campaignId: string): EmailCampaign {
    ...
    logger.info({ type: "campaign_cancelled", campaignId }, `Campaña cancelada: ${campaign.name}`);
  ...
  recordOpen(campaignId: string, email: string): void {
    ...
    logger.debug({ type: "campaign_opened", campaignId, email }, "Email abierto");
  }
  ...
  recordClick(campaignId: string, email: string, url: string): void {
    ...
    logger.debug({ type: "campaign_clicked", campaignId, email, url }, "Link clickeado");
  }
