...
export class PerpetualExcellenceManager {
  ...
  constructor() {
    logger.debug({ type: "perpetual_excellence_init" }, "Manager inicializado");
  }
  ...
  recordMetricValue(frameworkId: string, metricId: string, value: number): ExcellenceMetric | null {
  ...
  getStatistics(): Record<string, any> {
    ...
    return {
      ...
      metricsAchievingPercentage:
        allMetrics.length > 0 ? (achievingTarget.length / allMetrics.length) * 100 : 0,
      metricsByCategory: {
        quality: allMetrics.filter((m) => m.category === "quality").length,
        performance: allMetrics.filter((m) => m.category === "performance").length,
        reliability: allMetrics.filter((m) => m.category === "reliability").length,
        innovation: allMetrics.filter((m) => m.category === "innovation").length,
      },
    };
  }
