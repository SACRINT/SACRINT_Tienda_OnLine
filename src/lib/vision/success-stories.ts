...
export class SuccessStoryManager {
  ...
  featureStory(storyId: string): SuccessStory | null {
    ...
    story.featured = true;
    logger.info({ type: "story_featured", storyId }, `Historia destacada: ${story.storyTitle}`);
    return story;
  }
  ...
  getStatistics(): Record<string, any> {
    ...
    return {
      ...
      storiesByCategory: {
        ...
        innovation: stories.filter((s) => s.storyCategory === "innovation").length,
        operational: stories.filter((s) => s.storyCategory === "operational").length,
      },
      ...
      averageAdoptionLevel:
        practices.length > 0
          ? practices.reduce((sum, p) => sum + p.adoptionLevel, 0) / practices.length
          : 0,
      averageMaturityLevel:
        practices.length > 0
          ? practices.reduce((sum, p) => sum + p.maturityLevel, 0) / practices.length
          : 0,
    };
  }
