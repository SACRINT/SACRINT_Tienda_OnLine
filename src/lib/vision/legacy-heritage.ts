...
export interface LegacyDocument {
  ...
  documentType: "case-study" | "whitepaper" | "best-practice" | "lessons-learned";
  createdDate: Date;
  contributor: string;
  content: string;
  impact: string;
...
export class LegacyHeritageManager {
  ...
  getStatistics(): Record<string, any> {
    ...
    return {
      ...
      documentsByType: {
        caseStudy: documents.filter((d) => d.documentType === "case-study").length,
        whitepaper: documents.filter((d) => d.documentType === "whitepaper").length,
        bestPractice: documents.filter((d) => d.documentType === "best-practice").length,
        lessonsLearned: documents.filter((d) => d.documentType === "lessons-learned").length,
      },
