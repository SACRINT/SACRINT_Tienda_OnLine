...
export class ProjectRetrospectiveManager {
  ...
  constructor() {
    logger.debug({ type: "project_retrospective_init" }, "Manager inicializado");
  }
  ...
  getStatistics(): Record<string, any> {
    ...
    const totalObjectives = phases.reduce((sum, p) => sum + p.objectivesAchieved.length, 0);
    const totalChallenges = phases.reduce((sum, p) => sum + p.challengesFaced.length, 0);

    return {
      totalProjectPhases: phases.length,
      totalObjectivesAchieved: totalObjectives,
      totalChallengesFaced: totalChallenges,
      objectivesChallengeRatio: totalChallenges > 0 ? totalObjectives / totalChallenges : 0,
      totalLessonsLearned: phases.reduce((sum, p) => sum + p.lessonsLearned.length, 0),
      ...
      averageStakeholderSatisfaction:
        assessments.length > 0
          ? assessments.reduce((sum, a) => sum + a.stakeholderSatisfaction, 0) / assessments.length
          : 0,
      projectStatusDistribution: {
        completed: assessments.filter((a) => a.projectStatus === "completed").length,
        successful: assessments.filter((a) => a.projectStatus === "successful").length,
        exceedsExpectations: assessments.filter((a) => a.projectStatus === "exceeds-expectations")
          .length,
      },
      legacyStatementsCreated: legacies.length,
      averageHistoricalSignificance:
        legacies.length > 0
          ? legacies.reduce((sum, l) => sum + l.historicalSignificance, 0) / legacies.length
          : 0,
    };
  }
...
let globalProjectRetrospectiveManager: ProjectRetrospectiveManager | null = null;
