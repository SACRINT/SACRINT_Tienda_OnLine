...
export class IndustryLeadershipManager {
  ...
  getStatistics(): Record<string, unknown> {
    ...
    const averageCurrentRank =
      positions.length > 0
        ? positions.reduce((sum, p) => sum + p.currentRank, 0) /
          positions.length
        : 0

    const averageTargetRank =
      positions.length > 0
        ? positions.reduce((sum, p) => sum + p.targetRank, 0) /
          positions.length
        : 0
    ...
    return {
      ...
      positionsByCategory: {
        technology: positions.filter((p) => p.category === "technology")
          .length,
        innovation: positions.filter((p) => p.category === "innovation")
          .length,
        sustainability: positions.filter((p) => p.category === "sustainability")
          .length,
      ...
      awardsByRecognitionLevel: {
        ...
        regional: awards.filter((a) => a.recognitionLevel === "regional")
          .length,
        national: awards.filter((a) => a.recognitionLevel === "national")
          .length,
        international: awards.filter(
          (a) => a.recognitionLevel === "international"
        ).length,
      },
