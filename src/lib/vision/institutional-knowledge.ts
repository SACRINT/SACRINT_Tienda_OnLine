...
export class InstitutionalKnowledgeManager {
  ...
  constructor() {
    logger.debug(
      { type: "institutional_knowledge_init" },
  ...
  getStatistics(): Record<string, unknown> {
    ...
    return {
      ...
      totalExpertiseYears: profiles.reduce(
        (sum, p) => sum + p.yearsExperience,
        0
      ),
    }
  }
...
let globalInstitutionalKnowledgeManager: InstitutionalKnowledgeManager | null =
  null
