...
export class TechnicalDebtManager {
  ...
  assignDebtItem(debtItemId: string, owner: string, targetDate: Date): TechnicalDebtItem | null {
    ...
    logger.info(
      { type: "debt_item_assigned", itemId: debtItemId },
      `Deuda asignada a ${owner}`
    )
    return item
  }
  ...
  createRefactoringPlan(
    ...
    logger.info(
      { type: "refactoring_plan_created", planId: id },
  ...
  startRefactoring(debtItemId: string): TechnicalDebtItem | null {
    ...
    this.debtItems.set(debtItemId, item)
    logger.info(
      { type: "refactoring_started", itemId: debtItemId },
  ...
  completeDebtResolution(debtItemId: string): TechnicalDebtItem | null {
    ...
    logger.info(
      { type: "debt_resolved", itemId: debtItemId },
  ...
  getStatistics(): Record<string, unknown> {
    ...
    return {
      ...
      averageROI:
        items.length > 0
          ? items.reduce((sum, d) => sum + d.roi, 0) / items.length
          : 0,
