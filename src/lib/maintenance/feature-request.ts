...
export class FeatureRequestManager {
  ...
  addCommentToFeature(featureId: string, comment: string): FeatureRequest | null {
    ...
    this.featureRequests.set(featureId, feature)
    logger.info(
      { type: "feature_comment_added", featureId },
      `Comentario agregado al feature`
    )
    return feature
  }
  ...
  approveFeature(
    ...
    this.featureRequests.set(featureId, feature)
    logger.info(
      { type: "feature_approved", featureId },
      `Feature aprobado para ${targetRelease}`
    )
    return feature
  }
  ...
  completeFeature(featureId: string): FeatureRequest | null {
    ...
    this.featureRequests.set(featureId, feature)
    logger.info(
      { type: "feature_completed", featureId },
      `Feature completado`
    )
    return feature
  }
  ...
  getStatistics(): Record<string, unknown> {
    ...
    return {
      ...
      averageVotes:
        features.length > 0
          ? features.reduce((sum, f) => sum + f.votes, 0) / features.length
          : 0,
