...
export class CodeReviewManager {
  ...
  addReviewComment(
    ...
    logger.info(
      { type: "review_comment_added", reviewId },
  ...
  approveReview(reviewId: string, reviewer: string): CodeReview | null {
    ...
    this.codeReviews.set(reviewId, review)
    logger.info(
      { type: "code_review_approved", reviewId },
      `Code review aprobado por ${reviewer}`
    )
    return review
  }
  ...
  requestChanges(reviewId: string, reviewer: string): CodeReview | null {
    ...
    this.codeReviews.set(reviewId, review)
    logger.info(
      { type: "changes_requested", reviewId },
      `Cambios solicitados por ${reviewer}`
    )
    return review
  }
  ...
  getStatistics(): Record<string, unknown> {
    ...
    return {
      ...
      reviewsByStatus: {
        ...
        changesRequested: reviews.filter((r) => r.status === "changes-requested")
          .length,
