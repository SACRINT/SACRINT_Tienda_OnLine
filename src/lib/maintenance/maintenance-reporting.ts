...
export class MaintenanceReportingManager {
    ...
    const metrics: MaintenanceMetrics = {
      ...
      incidentTrend:
        this.historicalMetrics.length > 0
          ? this.historicalMetrics[this.historicalMetrics.length - 1].effectivenessScore <
            this.calculateCurrentEffectiveness(systemUptime)
            ? "improving"
            : "declining"
          : "stable",
  ...
  getReportByPeriod(period: string): MaintenanceReport | null {
    return (
      Array.from(this.maintenanceReports.values()).find((r) => r.reportingPeriod === period) || null
    );
  }
  ...
  getStatistics(): Record<string, unknown> {
    ...
    const avgUptime =
      reports.length > 0 ? reports.reduce((sum, r) => sum + r.systemUptime, 0) / reports.length : 0;
    ...
    return {
      ...
      totalIncidentsAcrossReports: reports.reduce((sum, r) => sum + r.totalIncidents, 0),
      averageResolutionRate:
        reports.length > 0
          ? reports.reduce((sum, r) => sum + (r.resolvedIncidents / r.totalIncidents) * 100, 0) /
            reports.length
          : 0,
    };
  }
