...
export class UserFeedbackManager {
  ...
  submitFeedback(
    ...
    logger.info({ type: "feedback_submitted", feedbackId: id }, `Feedback enviado: ${title}`);
    return feedbackItem;
  }
  ...
  respondToFeedback(
    ...
    this.feedback.set(feedbackId, item);
    logger.info({ type: "feedback_responded", feedbackId }, `Respuesta enviada al feedback`);
    return response;
  }
  ...
  generateAnalytics(): FeedbackAnalytics {
    ...
    const feedbackByType: Record<string, number> = {
      "bug-report": feedbackItems.filter((f) => f.feedbackType === "bug-report").length,
      "feature-request": feedbackItems.filter((f) => f.feedbackType === "feature-request").length,
      improvement: feedbackItems.filter((f) => f.feedbackType === "improvement").length,
    ...
    const resolutionRate = feedbackItems.length > 0 ? (resolved / feedbackItems.length) * 100 : 0;
  ...
  getStatistics(): Record<string, any> {
    ...
    return {
      ...
      feedbackByType: {
        ...
        featureRequest: items.filter((f) => f.feedbackType === "feature-request").length,
        improvement: items.filter((f) => f.feedbackType === "improvement").length,
        complaint: items.filter((f) => f.feedbackType === "complaint").length,
      },
      averageRating:
        items.length > 0 ? items.reduce((sum, f) => sum + f.rating, 0) / items.length : 0,
    };
  }
