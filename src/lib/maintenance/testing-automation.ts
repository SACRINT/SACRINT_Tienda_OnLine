...
export class TestingAutomationManager {
  ...
  addTestCase(suiteId: string, testName: string, expectedResult: string): TestCase | null {
    ...
    logger.info({ type: "test_case_added", testId: id }, `Caso de prueba agregado: ${testName}`);
    return testCase;
  }
