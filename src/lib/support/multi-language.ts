...
export class MultiLanguageSupportManager {
   ...
   */
  registerTranslation(key: string, language: string, value: string, context?: string): TranslationKey {
  ...
  translateObject<T extends Record<string, any>>(obj: T, fields: string[], language: string): T {
    ...
    for (const field of fields) {
      const key = `${obj.constructor.name}.${field}`
      translated[field] = this.getTranslation(key, language)
    }
   ...
   * Traducir ticket
   */
  translateTicket(ticketId: string, subject: string, description: string, targetLanguage: string): TicketTranslation | null {
    ...
    logger.info(
      { type: 'ticket_translated', ticketId, targetLanguage, quality: Math.round(translation.translationQuality) },
...
- Calidad Promedio: ${
      this.ticketTranslations.size > 0
        ? (
            Array.from(this.ticketTranslations.values()).reduce((sum, t) => sum + t.translationQuality, 0) /
