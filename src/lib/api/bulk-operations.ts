...
export class BulkOperationsManager {
  ...
  startExport(request: ExportRequest): ExportJob {
    try {
      ...
      logger.info(
        { type: 'export_started', exportId: job.id, entityType: request.entityType, format: request.format },
        `Exportación iniciada: ${job.id}`,
      )

      return job
    } catch (error) {
      logger.error({ type: 'export_start_error', error: String(error) }, 'Error al iniciar exportación')
      throw error
    }
  }
  ...
  startBulkOperation(request: BulkOperationRequest): BulkOperationJob {
    try {
      ...
      logger.info(
        { type: 'bulk_operation_started', operationId: job.id, operation: request.operation, recordCount: request.records.length },
        `Operación en masa iniciada: ${job.id}`,
      )

      return job
    } catch (error) {
      logger.error({ type: 'bulk_operation_start_error', error: String(error) }, 'Error al iniciar operación en masa')
      throw error
    }
  }
   ...
   */
  processBulkRecord(operationId: string, recordIndex: number, success: boolean, error?: string): void {
    ...
    if (job.processedRecords === job.totalRecords) {
      ...
      logger.info(
        { type: 'bulk_operation_completed', operationId, success: job.successCount, failed: job.failureCount },
  ...
  createImportTemplate(template: ImportTemplate): void {
    try {
      ...
      logger.info(
        { type: 'import_template_created', templateId: template.id, entityType: template.entityType },
        `Plantilla de importación creada: ${template.name}`,
      )
    } catch (error) {
      logger.error({ type: 'import_template_error', error: String(error) }, 'Error al crear plantilla')
    }
  }
  ...
  exportAsCSV(records: Record<string, any>[], fields: string[]): string {
    ...
    const rows = records.map((record) => fields.map((field) => this.escapeCSV(String(record[field] || ''))).join(','))
   ...
   * Validar importación
   */
  validateImportData(data: Record<string, any>[], template: ImportTemplate): { valid: boolean; errors: string[] } {
    const errors: string[] = []
