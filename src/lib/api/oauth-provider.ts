...
export class OAuthProviderManager {
  ...
  registerProvider(config: OAuthConfig): void {
    try {
      ...
      logger.info(
        { type: "oauth_provider_registered", provider: config.provider },
        `OAuth provider registrado: ${config.provider}`,
      );
    } catch (error) {
      logger.error(
        { type: "oauth_registration_error", provider: config.provider, error: String(error) },
        "Error al registrar OAuth",
      );
    }
  }
  ...
  generateAuthorizationUrl(provider: OAuthProvider): { url: string; state: string } {
    ...
    logger.info(
      { type: "authorization_url_generated", provider },
      `URL de autorización generada para ${provider}`,
    );
  ...
  async exchangeCodeForToken(
    ...
    try {
      ...
      logger.info(
        { type: "code_exchanged", provider },
        `Código intercambiado por token: ${provider}`,
      );

      return token;
    } catch (error) {
      logger.error(
        { type: "token_exchange_error", provider, error: String(error) },
  ...
  async getUserInfo(provider: OAuthProvider, accessToken: string): Promise<OAuthUser> {
    try {
      ...
      logger.info(
        { type: "user_info_fetched", provider, userId: user.id },
      ...
      logger.error(
        { type: "user_info_error", provider, error: String(error) },
  ...
  async refreshToken(provider: OAuthProvider, refreshToken: string): Promise<OAuthToken> {
    try {
      ...
      logger.error(
        { type: "token_refresh_error", provider, error: String(error) },
  ...
  disconnectThirdParty(connectionId: string): boolean {
    try {
      ...
      logger.error(
        { type: "third_party_disconnection_error", connectionId, error: String(error) },
...
let globalOAuthProvider: OAuthProviderManager | null = null;

export function initializeOAuthProvider(): OAuthProviderManager {
  if (!globalOAuthProvider) {
    globalOAuthProvider = new OAuthProviderManager();
  }
  return globalOAuthProvider;
}

export function getOAuthProvider(): OAuthProviderManager {
  if (!globalOAuthProvider) {
