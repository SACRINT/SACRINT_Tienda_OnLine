...
export class GraphQLAPI {
  ...
  defineSchema(schema: GraphQLSchema): void {
    try {
      ...
      logger.error(
        { type: "schema_definition_error", error: String(error) },
        "Error al definir schema",
      );
      throw error;
    }
  }
  ...
  async executeQuery<T = any>(query: GraphQLQuery): Promise<GraphQLResponse<T>> {
    ...
    try {
      ...
      logger.error(
        { type: "graphql_execution_error", error: String(error) },
  ...
  addFieldResolver(
    typeName: string,
    fieldName: string,
    resolver: (parent: any, args: any) => any | Promise<any>,
  ): void {
