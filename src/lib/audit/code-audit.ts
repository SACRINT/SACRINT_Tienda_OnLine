...
export class CodeAuditManager {
  ...
  reportIssue(file: string, line: number, severity: string, category: string, message: string, suggestion: string): CodeIssue {
  ...
  getStatistics() {
    ...
    return {
      ...
      averageScore: Array.from(this.auditResults.values()).reduce((sum, r) => sum + r.score, 0) / Math.max(1, this.auditResults.size),
    }
  }
}
