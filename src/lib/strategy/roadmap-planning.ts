...
export class RoadmapPlanningManager {
  ...
  createRoadmapItem(
    ...
    logger.info({ type: "roadmap_item_created", itemId: id }, `Item de roadmap creado: ${title}`);
    return item;
  }
  ...
  assignToQuarter(itemId: string, quarter: string): void {
    ...
    logger.info({ type: "item_assigned_to_quarter", itemId, quarter }, `Item asignado a trimestre`);
  }

  getItemsByPhase(
    phase: "discovery" | "design" | "development" | "launch" | "optimize",
  ): RoadmapItem[] {
    return Array.from(this.roadmapItems.values()).filter((item) => item.phase === phase);
  }

  getItemsByPriority(priority: "critical" | "high" | "medium" | "low"): RoadmapItem[] {
    return Array.from(this.roadmapItems.values()).filter((item) => item.priority === priority);
  }
  ...
  getQuarterlyRoadmap(quarter: string): RoadmapItem[] {
    ...
    return itemIds
      .map((id) => this.roadmapItems.get(id))
      .filter((item) => item !== undefined) as RoadmapItem[];
  }
