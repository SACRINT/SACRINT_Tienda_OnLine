...
export class TimelinePlanningManager {
  ...
  addPhase(
    ...
    logger.info(
      { type: "phase_added", timelineId },
      `Fase agregada: ${name} (${duration} días)`
    )
    return phase
  }
  ...
  getStatistics(): Record<string, unknown> {
    ...
    const totalMilestones = timelines.reduce(
      (sum, t) => sum + t.milestones.length,
      0
    )
    ...
    return {
      ...
      byStatus: {
        ...
        inExecution: timelines.filter((t) => t.status === "in-execution")
          .length,
