...
export class CapacityPlanningManager {
  ...
  registerCapacity(resourceType: string, totalCapacity: number, unit: string = "units"): Capacity {
  ...
  updateUsedCapacity(capacityId: string, usedCapacity: number): Capacity | null {
    ...
    capacity.utilizationPercent = (usedCapacity / capacity.totalCapacity) * 100 || 0;
  ...
  createCapacityForecast(
    ...
    const recommendations =
      gapAnalysis > 0
        ...
        : [`Capacidad suficiente`, "Monitorear demanda futura", "Optimizar utilización actual"];
  ...
  getCapacityByResourceType(resourceType: string): Capacity | null {
    return (
      Array.from(this.capacities.values()).find((c) => c.resourceType === resourceType) || null
    );
  }

  getOverCapacitatedResources(): Capacity[] {
    return Array.from(this.capacities.values()).filter((c) => c.utilizationPercent > 80);
  }

  getUnderutilizedResources(): Capacity[] {
    return Array.from(this.capacities.values()).filter((c) => c.utilizationPercent < 50);
  }
  ...
  getStatistics(): Record<string, any> {
    ...
    return {
      ...
      overCapacitatedCount: capacities.filter((c) => c.utilizationPercent > 80).length,
      underutilizedCount: capacities.filter((c) => c.utilizationPercent < 50).length,
