...
export class InitiativePrioritizationManager {
  ...
  constructor() {
    logger.debug({ type: "initiative_prioritization_init" }, "Manager inicializado");
  }
  ...
  calculatePriority(businessValue: number, effort: number, risk: number): number {
    const effortNormalized = Math.min(effort / 10, 1);
    const riskNormalized = Math.min(risk / 10, 1);
    return Math.round((businessValue * 0.6 - effortNormalized * 0.3 - riskNormalized * 0.1) * 100);
  }

  reprioritize(): Initiative[] {
    const sorted = Array.from(this.initiatives.values()).sort((a, b) => b.priority - a.priority);
  ...
  getInitiativesByAlignment(alignment: "strategic" | "tactical" | "operational"): Initiative[] {
    return Array.from(this.initiatives.values()).filter((i) => i.alignment === alignment);
  }
  ...
  getStatistics(): Record<string, any> {
    ...
    return {
      ...
      byAlignment: {
        strategic: initiatives.filter((i) => i.alignment === "strategic").length,
        tactical: initiatives.filter((i) => i.alignment === "tactical").length,
        operational: initiatives.filter((i) => i.alignment === "operational").length,
      },
    };
  }
...
let globalInitiativePrioritizationManager: InitiativePrioritizationManager | null = null;
