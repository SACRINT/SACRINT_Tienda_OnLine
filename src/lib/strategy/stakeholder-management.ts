...
export class StakeholderManagementManager {
  ...
  constructor() {
    logger.debug({ type: "stakeholder_management_init" }, "Manager inicializado");
  }
  ...
  recordEngagement(
    ...
    logger.info({ type: "engagement_recorded", stakeholderId }, `Engagement registrado: ${type}`);
    return engagement;
  }

  getStakeholdersByInterest(level: "critical" | "high" | "medium" | "low"): Stakeholder[] {
    return Array.from(this.stakeholders.values()).filter((s) => s.interestLevel === level);
  }

  getStakeholdersByInfluence(level: "critical" | "high" | "medium" | "low"): Stakeholder[] {
    return Array.from(this.stakeholders.values()).filter((s) => s.influenceLevel === level);
  }
  ...
  getStatistics(): Record<string, any> {
    ...
    return {
      ...
      byInterestLevel: {
        critical: stakeholders.filter((s) => s.interestLevel === "critical").length,
      ...
      byInfluenceLevel: {
        critical: stakeholders.filter((s) => s.influenceLevel === "critical").length,
      ...
      totalEngagements: stakeholders.reduce((sum, s) => sum + s.engagementHistory.length, 0),
    };
  }
...
let globalStakeholderManagementManager: StakeholderManagementManager | null = null;
