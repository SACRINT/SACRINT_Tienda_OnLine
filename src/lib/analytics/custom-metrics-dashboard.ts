...
export class CustomMetricsDashboardManager {
  ...
  constructor() {
    logger.debug(
      { type: "custom_metrics_dashboard_init" },
      "Custom Metrics Dashboard inicializado",
    );
  }
  ...
  addMetric(tenantId: string, metric: CustomMetricsDashboard["metrics"][number]): void {
  ...
  updateKPI(tenantId: string, kpiName: string, current: number, target: number): void {
    const dashboard = this.dashboards.get(tenantId);
    if (!dashboard) return;
