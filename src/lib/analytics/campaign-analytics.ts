...
export class CampaignAnalytics {
  ...
  generateTopCampaigns(
    metric: "openRate" | "clickRate" | "revenue",
    limit: number = 10,
  ): Array<{
    campaignName: string;
    value: number;
    ...
    return Array.from(this.metrics.values())
      .sort((a, b) => {
        const aValue =
          metric === "openRate" ? b.rates.openRate : metric === "clickRate" ? b.rates.clickRate : 0;
        const bValue =
          metric === "openRate" ? a.rates.openRate : metric === "clickRate" ? a.rates.clickRate : 0;
      ...
      .map((m) => ({
        ...
        value:
          metric === "openRate" ? m.rates.openRate : metric === "clickRate" ? m.rates.clickRate : 0,
      }));
  }
