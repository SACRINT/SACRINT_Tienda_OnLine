...
export const securityAudit = {
  ...
  authFailure: (request: Request, email: string, reason: string) => {
    const { ipAddress, userAgent } = getRequestInfo(request);
    return logSecurityEvent({
      type: "auth_failure",
      ipAddress,
  ...
  passwordChange: (request: Request, userId: string) => {
    const { ipAddress, userAgent } = getRequestInfo(request);
    return logSecurityEvent({
  ...
  rateLimitExceeded: (request: Request, identifier: string) => {
    const { ipAddress, userAgent } = getRequestInfo(request);
    return logSecurityEvent({
  ...
  dataExport: (request: Request, userId: string, dataType: string) => {
    const { ipAddress, userAgent } = getRequestInfo(request);
    return logSecurityEvent({
      type: "data_export",
      userId,
