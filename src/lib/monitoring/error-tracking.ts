...
export class ErrorTrackingManager {
  ...
  trackError(message: string, severity: string, service: string, context?: Record<string, any>): ErrorEvent {
    ...
    const fingerprint = Buffer.from(message + service).toString("base64").substring(0, 32)
