...
export class HealthCheckMonitor {
  ...
  async runCheck(name: string): Promise<HealthCheckResult> {
    ...
    try {
      ...
      if (config.critical) {
        addBreadcrumb(`Critical service unhealthy: ${name}`, {
          category: "health",
          level: "error",
  ...
  stopMonitoring(): void {
    ...
    logger.info({ type: "health_monitoring_stopped" }, "Health monitoring detenido");
  }
  ...
  calculateUptime(name: string): number {
    ...
    const healthy = history.filter((r) => r.status === "healthy" || r.status === "degraded").length;
  ...
  async getHealthSummary(): Promise<HealthCheckSummary> {
    ...
    const allDegraded = results.every((r) => r.status === "healthy" || r.status === "degraded");
    ...
    const uptimes = Array.from(this.checks.keys()).map((name) => this.calculateUptime(name));
    const avgUptime = uptimes.length > 0 ? uptimes.reduce((a, b) => a + b) / uptimes.length : 100;
...
export const CommonHealthChecks = {
   ...
   */
  externalAPI: (endpoint: string, timeout: number = 5000): HealthCheckConfig => ({
