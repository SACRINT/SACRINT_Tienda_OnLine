...
export class ErrorRateMonitor {
  ...
  constructor(thresholds?: Partial<typeof ErrorRateMonitor.prototype.thresholds>) {
    if (thresholds) {
  ...
  recordError(record: ErrorRecord): void {
    ...
    this.severityCounts.set(
      record.severity,
      (this.severityCounts.get(record.severity) || 0) + 1,
    )
    this.sourceCounts.set(
      record.source,
      (this.sourceCounts.get(record.source) || 0) + 1,
    )
  ...
  getStats(): ErrorStats {
    ...
    return {
      ...
      criticalErrors: Array.from(this.severityCounts.get('critical') || 0),
...
export function initializeErrorMonitor(
  thresholds?: Partial<typeof ErrorRateMonitor.prototype.thresholds>,
): ErrorRateMonitor {
  if (!globalMonitor) {
