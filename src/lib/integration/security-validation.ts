...
export class SecurityValidationManager {
  ...
  createSecurityCheck(category: string, checkName: string, severity: string = "medium"): SecurityCheck {
  ...
  generateSecurityReport(): SecurityValidationReport {
    ...
    logger.info({ type: "report_generated" }, `Security report: ${report.canDeployToProd ? "SAFE" : "UNSAFE"}`)
    return report
  }
