...
export class TroubleshootingGuideManager {
  ...
  createIssue(title: string, symptoms: string[], rootCauses: string[], solutions: string[]): TroubleshootingIssue {
  ...
  findIssueBySymptom(symptom: string): TroubleshootingIssue[] {
    return Array.from(this.issues.values()).filter(i => i.symptoms.some(s => s.includes(symptom)))
  }
