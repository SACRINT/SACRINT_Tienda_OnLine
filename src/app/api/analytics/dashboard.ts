...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.error({ type: 'analytics_fetch_error', error: String(error) }, 'Error al obtener analytics')
...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.error({ type: 'dashboard_creation_error', error: String(error) }, 'Error al crear dashboard')
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
