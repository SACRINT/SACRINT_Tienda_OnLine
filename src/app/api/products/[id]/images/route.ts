...
import { addProductImage, removeProductImage, reorderProductImages } from '@/lib/db/products'
...
export async function DELETE(
  ...
  try {
    ...
    if (role !== USER_ROLES.STORE_OWNER && role !== USER_ROLES.SUPER_ADMIN) {
      return NextResponse.json(
        { error: 'Forbidden - Only STORE_OWNER or SUPER_ADMIN can remove images' },
        { status: 403 }
      )
    }
    ...
    console.log(`[PRODUCTS] Removed image ${imageId} from product ${productId}`)
...
export async function PATCH(
  ...
  try {
    ...
    if (role !== USER_ROLES.STORE_OWNER && role !== USER_ROLES.SUPER_ADMIN) {
      return NextResponse.json(
        { error: 'Forbidden - Only STORE_OWNER or SUPER_ADMIN can reorder images' },
        { status: 403 }
      )
    }
