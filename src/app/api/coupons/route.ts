...
import { CreateCouponSchema, CouponFilterSchema } from '@/lib/security/schemas/coupon-schemas'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (role !== USER_ROLES.STORE_OWNER && role !== USER_ROLES.SUPER_ADMIN) {
      return NextResponse.json(
        { error: 'Forbidden - Only STORE_OWNER or SUPER_ADMIN can create coupons' },
        { status: 403 }
      )
    }
