...
describe("Sanitization Utilities", () => {
  ...
  describe("sanitizeRedirectUrl", () => {
    ...
    it("should allow configured hosts", () => {
      const url = "https://trusted.com/path"
      const result = sanitizeRedirectUrl(url, ["trusted.com"])
...
describe("Rate Limiting", () => {
  ...
  describe("checkRateLimit", () => {
    ...
    it("should reset after window expires", () => {
      const config = { max: 1, windowMs: 1000, identifier: "test" }

      // Use up the limit
      checkRateLimit("test-key", config)

      // Should be blocked
      expect(checkRateLimit("test-key", config).success).toBe(false)

      // Advance time past window
      vi.advanceTimersByTime(1500)

      // Should be allowed again
      expect(checkRateLimit("test-key", config).success).toBe(true)
    })
  ...
  describe("rateLimitConfigs", () => {
    ...
    it("should have api config with higher limit", () => {
      expect(rateLimitConfigs.api.max).toBeGreaterThan(rateLimitConfigs.auth.max)
    })
  })
})
