...
```css
--primary: #0A1128      /* Navy blue - main brand */
--accent: #D4AF37       /* Gold - highlights */
--mint: #8FBC8F         /* Mint green - secondary */
...
<PaymentForm
  amount={599.98}
  onPaymentComplete={(result) => handleComplete(result)}
/>
```
...
function MyComponent() {
  ...
  return (
    <button onClick={() => success('¡Guardado!')}>
      Guardar
    </button>
  )
}
